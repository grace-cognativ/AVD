...
| TC-ID | Test Case | Expected Result | Priority | Results |
|-------|-----------|-----------------|----------|---------|
| AUTH-001 | Request without Authorization header | 401 Unauthorized with appropriate error message | P0 | **Error: response status is 401 without error message** |
| AUTH-002 | Request with invalid JWT token format | 401 Unauthorized with validation error | P0 |
| AUTH-003 | Request with expired JWT token | 401 Unauthorized with expiration error | P0 |
| AUTH-004 | Request with valid JWT token in Authorization header | Request processed successfully | P0 |
| AUTH-005 | Request with JWT token missing required claims (sub, scope, iss, aud) | 401 Unauthorized with missing claim error | P0 |
| AUTH-006 | Request with JWT token from incorrect issuer | 401 Unauthorized with issuer validation error | P0 |
| AUTH-007 | Request with JWT token for incorrect audience | 401 Unauthorized with audience validation error | P0 |
| AUTH-008 | Health check endpoints without authentication | Request succeeds (health endpoints are public) | P1 |
...
|-------|-----------|-----------------|----------|
| AUTH-009 | STP endpoint with token missing `addvantage.stp` scope | 403 Forbidden (if policy enforced) | P1 |
| AUTH-010 | Inquiry endpoint with token missing `addvantage.inquiry` scope | 403 Forbidden (if policy enforced) | P1 |
| AUTH-011 | Request with token having appropriate scope | Request processed successfully | P0 |
...
| TC-ID | Test Case | Expected Result | Priority |
|-------|-----------|-----------------|----------|
| STP-001 | Valid STP request with all required fields | 200 OK with success response containing transactionId, status, processedAt, legacyResponse | P0 |
| STP-002 | STP request with missing `operation` field | 400 Bad Request with validation error for operation field | P0 |
| STP-003 | STP request with missing `fields` dictionary | 400 Bad Request with validation error for fields | P0 |
| STP-004 | STP request with empty `fields` dictionary | 400 Bad Request with validation error indicating fields cannot be empty | P0 |
| STP-005 | STP request with invalid operation name | 404 Not Found with FormatNotFound error | P0 |
| STP-006 | STP request missing `AddVantage-Authorization` header | 400 Bad Request with validation error for missing header | P0 |
| STP-007 | STP request missing `uuid` header | 400 Bad Request with validation error for missing UUID header | P0 |
| STP-008 | STP request with invalid UUID format | 400 Bad Request with validation error for invalid UUID format | P0 |
| STP-009 | STP request with invalid `AddVantage-Authorization` format | 400 Bad Request with validation error | P0 |
| STP-010 | STP request with fields failing format validation rules | 400 Bad Request with field-specific validation errors | P0 |
| STP-011 | STP request with non-JSON Content-Type | 400 Bad Request or 415 Unsupported Media Type | P1 |
| STP-012 | STP request with malformed JSON body | 400 Bad Request with JSON parsing error | P0 |
| STP-013 | STP request when Format Management Service is unavailable | 502 Bad Gateway with ExternalServiceError | P1 |
...
| TC-ID | Test Case | Expected Result | Priority |
|-------|-----------|-----------------|----------|
| BATCH-001 | Valid batch request with multiple items | 200 OK with batch response containing all items processed | P0 |
| BATCH-002 | Batch request with empty items array | 400 Bad Request with error indicating batch cannot be empty | P0 |
| BATCH-003 | Batch request with null items | 400 Bad Request with error indicating batch cannot be empty | P0 |
| BATCH-004 | Batch request exceeding MaxBatchSize (100 items) | 429 Too Many Requests with rate limit error | P0 |
| BATCH-005 | Batch request with invalid item at index 0 | 400 Bad Request with validation errors | P0 |
| BATCH-006 | Batch request with invalid item at middle index | 200 OK with partial success, failedAtIndex set, validationErrors populated | P0 |
| BATCH-007 | Batch request with batchId provided | 200 OK with same batchId in response | P1 |
| BATCH-008 | Batch request when batch processing timeout exceeds | 504 Gateway Timeout with timeout error | P1 |
| BATCH-009 | Batch request with all items succeeding | 200 OK with success: true, failedAtIndex: null | P0 |
| BATCH-010 | Batch request with mixed success/failure | 200 OK with success: false, failedAtIndex set, legacyResponses for successful items | P0 |
...
| TC-ID | Test Case | Expected Result | Priority |
|-------|-----------|-----------------|----------|
| INQ-001 | Valid inquiry request with all required parameters | 200 OK with success response containing data object | P0 |
| INQ-002 | Inquiry request with missing `endpoint` field | 400 Bad Request with validation error for endpoint field | P0 |
| INQ-003 | Inquiry request with missing `parameters` object | 400 Bad Request with validation error for parameters | P0 |
| INQ-004 | Inquiry request with invalid endpoint name | 404 Not Found with FormatNotFound error | P0 |
| INQ-005 | Inquiry request with invalid query parameters | 400 Bad Request with parameter-specific validation errors | P0 |
| INQ-006 | Inquiry request missing `AddVantage-Authorization` header | 400 Bad Request with validation error for missing header | P0 |
| INQ-007 | Inquiry request missing `uuid` header | 400 Bad Request with validation error for missing UUID header | P0 |
| INQ-008 | Inquiry request with parameters not matching format definition | 400 Bad Request with validation errors | P0 |
| INQ-009 | Inquiry request when Format Management Service is unavailable | 502 Bad Gateway with ExternalServiceError | P1 |
| INQ-010 | Inquiry request when Legacy Service is unavailable | 502 Bad Gateway with ExternalServiceError | P1 |
| INQ-011 | Inquiry request with empty parameters object | 400 Bad Request or successful (depending on endpoint requirements) | P1 |
